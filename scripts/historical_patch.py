...
def fetch_weather_data(latitude, longitude, date_to_fetch):
    date_str = date_to_fetch.strftime('%Y-%m-%d')
    days_ago = (datetime.now() - date_to_fetch).days
    if days_ago < 3:
        print(f"Fetching recent weather for {date_str} from FORECAST API...")
        ...
        print(f"Fetching older weather for {date_str} from ARCHIVE API...")
...
def transform_historical_data(date_to_fetch, aqi_history_data, weather_data):
    ...
    try:
        ...
        temp = daily_weather.get('temperature_2m_mean', [None])[-1]
        precip = daily_weather.get('precipitation_sum', [None])[-1]
        wind = daily_weather.get('wind_speed_10m_max', [None])[-1]
...
if __name__ == "__main__":
    print("--- Starting Data Patch Process for Missing Dates ---")
    ...
    if not engine:
        ...
        for date_to_fetch in dates_to_process:
            print(f"\n{'='*20} Processing Date: {date_to_fetch.strftime('%Y-%m-%d')} {'='*20}")
            for location in LOCATIONS:
                print(f"\n--- Fetching for {location['city']} ---")
                aqi_data = fetch_historical_aqi_data(location['latitude'], location['longitude'], date_to_fetch)
                
                # --- THE FIX: Pass the datetime object, not the string ---
                weather_data = fetch_weather_data(location['latitude'], location['longitude'], date_to_fetch)
